...
object CORS {
  ...
  /**
    ...
    */
  def apply[F[_]](http: LambdaApp[F], config: CORSConfig = DefaultCORSConfig)(
      implicit F: Applicative[F]): LambdaApp[F] =
    Kleisli { req =>
      ...
      def varyHeader(response: Response[F]): Response[F] =
      ...
      (req.req.method, req.req.headers.get(Origin), req.req.headers.get(`Access-Control-Request-Method`)) match {
        case (OPTIONS, Some(origin), Some(acrm)) if allowCORS(origin, acrm) =>
          logger.debug(s"Serving OPTIONS with CORS headers for $acrm ${req.req.uri}")
          createOptionsResponse(origin, acrm).pure[F]
        case (_, Some(origin), _) =>
          if (allowCORS(origin, Header("Access-Control-Request-Method", req.req.method.renderString))) {
            http(req).map { resp =>
              logger.debug(s"Adding CORS headers to ${req.req.method} ${req.req.uri}")
              corsHeaders(origin.value, req.req.method.renderString, isPreflight = false)(resp)
            }
          } else {
            logger.debug(s"CORS headers were denied for ${req.req.method} ${req.req.uri}")
